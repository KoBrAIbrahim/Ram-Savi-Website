...
// These MUST be defined outside Join so React doesn't treat them
// as new component types on every re-render (which would destroy
// the focused input and cause instant focus loss on each keystroke).
...
function SpinBtn({ label, loading, submittingLabel }) {
  return (
    <button
      type="submit"
      disabled={loading}
...
function Join({ language }) {
  ...
  const resetAll = () => {
    ...
    setFormData({ name: '', email: '', phone: '', businessType: 'FOODANDDRINK', businessName: '', city: '', message: '' })
                ...
                {joinType === 'savi' ? (
                  <form onSubmit={handleSaviSubmit} className="space-y-5">
                    <InputRow label={t.nameLabel}>
                      <input type="text" name="name" value={formData.name} onChange={handleChange} required placeholder={t.namePlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.emailLabel}>
                      <input type="email" name="email" value={formData.email} onChange={handleChange} required placeholder={t.emailPlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.phoneLabel}>
                      <input type="tel" name="phone" value={formData.phone} onChange={handleChange} required placeholder={t.phonePlaceholder} className="input-field" />
                    </InputRow>
                    ...
                    <InputRow label={t.businessNameLabel}>
                      <input type="text" name="businessName" value={formData.businessName} onChange={handleChange} required placeholder={t.businessNamePlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.cityLabel}>
                      <input type="text" name="city" value={formData.city} onChange={handleChange} required placeholder={t.cityPlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.messageLabel}>
                      <textarea name="message" value={formData.message} onChange={handleChange} rows={3} placeholder={t.messagePlaceholder} className="input-field resize-none" />
                    </InputRow>
                    <SpinBtn label={t.submit} loading={loading} submittingLabel={t.submitting} />
                    {error && <p className="text-red-600 text-sm text-center">{t.errorMessage}</p>}
                  </form>
                ) : (
                  <form onSubmit={handleDevSubmit} className="space-y-5">
                    <InputRow label={t.nameLabel}>
                      <input type="text" name="name" value={devFormData.name} onChange={handleDevChange} required placeholder={t.namePlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.emailLabel}>
                      <input type="email" name="email" value={devFormData.email} onChange={handleDevChange} required placeholder={t.emailPlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.phoneLabel}>
                      <input type="tel" name="phone" value={devFormData.phone} onChange={handleDevChange} required placeholder={t.phonePlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.companyNameLabel}>
                      <input type="text" name="companyName" value={devFormData.companyName} onChange={handleDevChange} required placeholder={t.companyNamePlaceholder} className="input-field" />
                    </InputRow>
                    <InputRow label={t.serviceTypeLabel}>
                      <select name="serviceType" value={devFormData.serviceType} onChange={handleDevChange} className="input-field">
                    ...
                    <InputRow label={t.projectDetailsLabel}>
                      <textarea name="projectDetails" value={devFormData.projectDetails} onChange={handleDevChange} required rows={5} placeholder={t.projectDetailsPlaceholder} className="input-field resize-none" />
                    </InputRow>
                    <SpinBtn label={t.devSubmit} loading={loading} submittingLabel={t.submitting} />
                    {error && <p className="text-red-600 text-sm text-center">{t.errorMessage}</p>}
                  </form>
                )}
              </>
